...
export default function Two() {
    ...
    const handleButtonClick2 = async (buttonText2) => {
      ...
      sessionStorage.setItem("widgetTwoTrue",`${widgetResponse.data.columnName}`)
      navigate(0)
    };
  ...
  return (
    <div className="widgetTwoButton">
      ...
      {["협곡 분석","협곡 티어", "칼바람 분석", "칼바람 티어"].map((buttonText2) => (
        <button
          key={buttonText2}
          type="button"
          class={`nav-button ${activeButton2 === buttonText2 ? "active" : ""}`}
